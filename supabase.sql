...
-- Teacher names master list (for Nama Guru suggestions)
create table if not exists public.nilam_teachers (
  nama_guru text primary key,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
drop trigger if exists trg_nilam_teachers_updated_at on public.nilam_teachers;
create trigger trg_nilam_teachers_updated_at
before update on public.nilam_teachers
for each row execute function public.set_updated_at();
...
alter table public.nilam_teachers enable row level security;
...
grant select, insert, update, delete on table public.nilam_teachers to anon;
...
drop policy if exists "allow_insert_update_select_for_anon_teachers" on public.nilam_teachers;
create policy "allow_insert_update_select_for_anon_teachers"
on public.nilam_teachers
for all
to anon
using (true)
with check (true);
